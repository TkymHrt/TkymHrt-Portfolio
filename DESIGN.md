...
- 明るい紙面と細い区切り線による、読み進めやすい一枚のプロフィール
- オレンジと黄色を要所に置く、節度あるフルパレット
- 角丸 8〜16px の道具的で触れやすい部品
- PC では一覧性を優先し、モバイルではヒーローを最初の一画面に広げてから一方向に読み進める構成
...
ロゴを左、短い英語ラベルを右に置く。現在地は `primary` の下線、ホバーは `ink` の濃度変化で示す。モバイルではロゴとテーマ切替だけを残し、本文内の明確な導線へ集約する。
