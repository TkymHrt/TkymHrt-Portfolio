		...
		<AdminLayout>
			<div className="min-h-screen bg-gray-100 p-4">
				...
				<main className="bg-white shadow-sm p-4">
					...
					<button onClick={addItem} className="bg-blue-500 text-white px-4 py-2 rounded-sm mb-4">
						アイテムを追加
					</button>
					<ul className="space-y-4">
						{portfolioItems.map((item) => (
							<li key={item.id} className="border p-4 rounded-sm shadow-sm">
								<h2 className="text-xl font-semibold">{item.title}</h2>
								<p className="text-gray-600">{item.description}</p>
								{/* biome-ignore lint/a11y/useButtonType: <explanation> */}
								<button onClick={() => deleteItem(item.id)} className="bg-red-500 text-white px-4 py-2 rounded-sm mt-2">
									削除
								</button>
							</li>
						))}
